...
const PILLARS_METADATA = [
  ...
  { id: 2, name: "Psicometría IPIP-NEO", icon: Brain, code: "02. PSICOLÓGICO" },
...
const PHASE_SUBTITLES = {
  ...
  2: "FASE 2: EVALUACIÓN PSICOMÉTRICA IPIP-NEO",
  3: "FASE 3: DILEMAS TÁCTICOS E INTERESES",
  4: "FASE 4: DICTAMEN VOCACIONAL OFICIAL",
};
  ...
  return (
    <div className="w-full bg-transparent">
      <div className="max-w-6xl mx-auto px-4 py-2.5 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <span className="font-teko text-xs uppercase font-bold text-peru-red tracking-widest leading-none truncate">
            ...
            <h2 className="font-rajdhani text-sm sm:text-base font-bold text-slate-900 dark:text-white tracking-wide uppercase truncate max-w-[210px] sm:max-w-none leading-none mt-0.5">
              {activeMeta.id === 2 ? (
        ...
        {/* Indicadores Tácticos de Pasos (1 a 4) - Se adaptan sin romper el layout */}
          ...
          {PILLARS_METADATA.map((p) => {
            ...
            return (
              <button
                ...
                className={`relative px-2 sm:px-2.5 md:px-3 py-1 rounded-lg font-rajdhani font-bold text-xs tracking-wider transition-all duration-200 flex items-center gap-1 sm:gap-1.5 select-none cursor-pointer whitespace-nowrap shrink-0 ${
                  isActive
              ...
              >
                <span>{p.id}</span>
                <span className="hidden md:inline font-teko text-xs tracking-widest">{p.code.split('.')[1].trim()}</span>
              </button>
            );
          })}
        </div>
